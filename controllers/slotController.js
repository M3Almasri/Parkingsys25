...
// Add this function to controllers/slotController.js
exports.updateSlotFromSensor = async (req, res) => {
  try {
    const { slot_id, sensor_status, timestamp } = req.body;

    // Validate input data
    if (!slot_id || !sensor_status) {
      return res.status(400).json({ 
        message: 'Missing required fields: slot_id and sensor_status' 
      });
    }

    // Validate sensor_status values
    if (!['occupied', 'available'].includes(sensor_status)) {
      return res.status(400).json({ 
        message: 'Invalid sensor_status. Must be "occupied" or "available"' 
      });
    }
    ...
    // Update slot availability based on sensor reading
    const is_available = sensor_status === 'available';
    
    // Only update if the status actually changed
    if (slot.is_available !== is_available) {
      const updatedSlot = await Slot.findOneAndUpdate(
        { slot_id: slot_id },
      ...
      console.log(`Sensor update: Slot ${slot_id} is now ${sensor_status}`);
      
      res.json({
        message: `Slot ${slot_id} status updated successfully`,
        slot: updatedSlot,
      ...
      res.json({
        message: `Slot ${slot_id} status unchanged`,
